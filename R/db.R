  ...
  switch(tipo,
    mysql = {
      if (!requireNamespace("RMySQL", quietly = TRUE)) {
        stop("Paquete 'RMySQL' no está instalado. Instálalo con: install.packages('RMySQL')", 
             call. = FALSE)
      }
      RMySQL::MySQL()
    },
    db2 = {
      if (!requireNamespace("RJDBC", quietly = TRUE)) {
        stop("Paquete 'RJDBC' no está instalado. Instálalo con: install.packages('RJDBC')", 
             call. = FALSE)
      }
    ...
    stop("Tipo de base de datos no soportado: '", tipo, "'.\n",
...
sbc_connect <- function(config, tipo) {
  ...
  tryCatch({
    ...
    stop(
      "Error al conectar a ", toupper(tipo), ":\n",
      "  Host: ", config$host, "\n",
...
sbc_disconnect <- function(conn) {
  ...
  # Mensaje informativo
  label <- toupper(tipo)
  if (dbname != "") label <- paste0(label, " (", dbname, ")")
  if (host != "") label <- paste0(label, " @ ", host)
...
sbc_query <- function(conn, 
  ...
  tryCatch({
    ...
    stop(
      ...
      "  SQL: ", substr(sql, 1, 100), if (nchar(sql) > 100) "..." else "", "\n",
      "  Error: ", e$message,
      call. = FALSE
    )
  })
...
sbc_with_connection <- function(config, query_fn) {
  ...
  if (!is.function(query_fn)) {
    stop("query_fn debe ser una función que reciba la conexión como argumento", 
         call. = FALSE)
  }
  ...
  if (!tipo %in% c("mysql", "db2")) {
    stop(
      "Tipo de base de datos no válido: '", tipo, "'\n",
      "Tipos válidos: 'mysql' o 'db2'",
      call. = FALSE
    )
  }
  ...
  tryCatch({
    ...
    stop(
      ...
      "  Tipo BD: ", toupper(tipo), "\n",
      "  Host: ", config$host %||% "N/A", "\n",
      "  Base de datos: ", config$dbname %||% "N/A", "\n",
      "  Error: ", e$message,
      call. = FALSE
    )
  }, finally = {
...
sbc_table_exists <- function(conn, nombre_tabla, ignorar_caso = TRUE) {
  ...
  tryCatch({
    ...
    warning(
      "Error al verificar existencia de tabla '", nombre_tabla, "':\n",
      "  Error: ", e$message
    )
    FALSE
  })
...
sbc_table_info <- function(conn, nombre_tabla) {
  ...
  tryCatch({
    ...
    stop(
      "Error al obtener información de tabla '", nombre_tabla, "':\n",
      "  Error: ", e$message,
      call. = FALSE
    )
  })
